...
df_loan_mei <- df_loan %>% filter(tanggal_cair >= '2020-05-01', tanggal_cair <= '2020-05-31') %>% 
  group_by(cabang) %>% summarise(total_amount = sum(amount))
df_loan_mei

library(scales)

df_loan_mei %>% arrange(desc(total_amount)) %>% 
  mutate(total_amount=comma(total_amount)) %>% head(5) 
...
ggplot(df_loan_mei_flag, aes(x = umur, y = total_amount)) +
  ...
  labs(title = "Ada cabang berpeforma rendah padahal tidak termasuk bottom 5 nasional",
       ...
       x = "Umur (bulan)",
       y = "Total Amount")
#performa cabang diumur yg sama
df_loan_mei_flag %>% 
  filter(umur == 3) %>% 
...
#performa agen pada cabang rendah
df_loan_mei_flag %>% 
